...
THolder<NActors::TActorSystemSetup> BuildActorSystemSetup(ui32 threads,
                                                          ui32 pools) {
  ...
  setup->Scheduler.Reset(
      new NActors::TBasicSchedulerThread(NActors::TSchedulerConfig(512, 0)));
  return setup;
}

int main(int argc, const char *argv[]) {
  Y_UNUSED(argc);
  auto setup = BuildActorSystemSetup(4, 1);
  ...
  actorSystem.Register(CreateReadActor(std::cin, writeId).Release());
