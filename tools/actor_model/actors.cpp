...
public:
    TReadActor(const NActors::TActorId writeActor)
        : WriteActor(writeActor), Finish(false), Count(0), HasEntered(false)
    {
    }
...
public:
    ...
    void HandleWakeup()
    {
        ...
        while (Value > 1)
        {
            while (Value % CurrentTestDevisor == 0)
            {
                Prime = CurrentTestDevisor;
                Value /= CurrentTestDevisor;
