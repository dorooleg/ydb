...
public:
    ...
    void HandleWakeup() {
        ...
        for (int64_t divisor = current_divisor; divisor <= sqrt(value); divisor+=(divisor==2?1:2)) {
            ...
            while (value % current_divisor == 0) {
                ...
                if(std::chrono::duration_cast<std::chrono::milliseconds>(elapsed_time - start_time).count() > 10){
                    Send(SelfId(), std::make_unique<NActors::TEvents::TEvWakeup>());
                    return;
                }
            }
