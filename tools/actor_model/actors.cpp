...
public:
  TReadActor(IInputStream &strm, const NActors::TActorId &writeActorId)
      : Strm(strm), WriteActorId(writeActorId) {}
  ...
  void HandleWakeup() {
    if (CurrentIndex < Numbers.size()) {
      ...
      auto actor =
          CreateMaximumPrimeDivisorActor(value, SelfId(), WriteActorId);
  ...
  void ParseAndQueueNumbers(const TString &line) {
    Numbers.clear();
    CurrentIndex = 0;
...
public:
  TMaximumPrimeDivisorActor(int64_t value, const NActors::TActorId &readActorId,
  ...
  void HandleWakeup() {
    ...
    if (ProcessSpecialCases()) {
      return;
    }

    bool completed = ContinueFactorization(start);
  ...
  bool ContinueFactorization(TInstant start) {
    int64_t n = Value < 0 ? -Value : Value;

    if (CurrentDivisor > 2 && MaxPrimeDivisor > 0) {
      n = Value / MaxPrimeDivisor;
      if (n == 1)
        return true;
    }

    while (CurrentDivisor * CurrentDivisor <= n &&
           (TInstant::Now() - start) < TDuration::MilliSeconds(10)) {
    ...
    if (n > 1 && (TInstant::Now() - start) < TDuration::MilliSeconds(10)) {
      MaxPrimeDivisor = std::max(MaxPrimeDivisor, n);
    }

    return CurrentDivisor * CurrentDivisor > n || n == 1;
  }

  void FinishCalculation() {
    Send(WriteActorId,
         std::make_unique<TEvents::TEvWriteValueRequest>(MaxPrimeDivisor));
...
CreateReadActor(IInputStream &strm, const NActors::TActorId &writeActorId) {
  return MakeHolder<TReadActor>(strm, writeActorId);
}

THolder<NActors::IActor>
CreateMaximumPrimeDivisorActor(int64_t value,
                               const NActors::TActorId &readActorId,
                               const NActors::TActorId &writeActorId) {
  return MakeHolder<TMaximumPrimeDivisorActor>(value, readActorId,
                                               writeActorId);
}
